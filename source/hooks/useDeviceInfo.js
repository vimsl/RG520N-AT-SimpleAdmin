...
import { getAtCommand, parseAtResponse } from '../utils/atParser';
...
export default function useDeviceInfo(lang) {
  ...
  const fetch_ = useCallback(async (signal) => {
    try {
      const params = new URLSearchParams({ atcmd: getAtCommand() });
      const resp = await fetch(`/cgi-bin/get_atcommand?${params}`, { signal: signal || AbortSignal.timeout(5000) });
